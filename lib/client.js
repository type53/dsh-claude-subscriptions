window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			oauthConnecting: "正在等待授权…（请在打开的窗口中完成登录）",
			oauthReconnect: "重新登录",
			...
			modelsTitle: "可用模型",
			modelsHint:
				"连接后，在对话输入框左侧的模型选择器（/model）中选择 Claude 模型即可使用；也可在 设置 → 模型 中查看该提供方。",
		...
		const en = {
			...
			oauthConnecting: "Waiting for authorization… (complete the login in the opened window)",
			...
			modelsHint:
				"Once connected, pick a Claude model from the model picker (/model) next to the input box; the provider also appears under Settings → Models.",
		...
		function SubscriptionSection(props) {
			...
			react.useEffect(() => {
				if (flow === undefined || flow.url === undefined || flow.url.length === 0) return;
				if (openedFlow === flow.flowId) return;
				setOpenedFlow(flow.flowId);
			...
			react.useEffect(() => {
				...
				if (flow.startedAt !== undefined && Date.now() - flow.startedAt > FLOW_TIMEOUT_MS) {
					cancelLogin();
					return undefined;
				}
			...
			const startLogin = async () => {
				...
				try {
					...
					const revision = snap?.ns?.revision;
					await mutateOnce(
						[{ op: "set", path: ["flow"], value: { flowId, startedAt: Date.now() } }],
						revision,
					);
					setOpenedFlow(null);
					await load();
			...
			const cancelLogin = async () => {
				try {
					await mutateOnce([{ op: "unset", path: ["flow"] }], snap?.ns?.revision);
					await load();
			...
			const disconnect = async () => {
				...
				try {
					...
					await mutateOnce([{ op: "unset", path: ["auth"] }], snap?.ns?.revision);
					await load();
				} catch (error) {
					setNotice(`${t("error")}: ${messageOf(error)}`);
				} finally {
			...
			const loginInFlight = flow !== undefined;
