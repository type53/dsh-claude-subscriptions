window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			statusTitle: "订阅状态",
			statusNone: "未检测到订阅凭据",
			statusPasted: "正在使用粘贴的令牌（setup-token）",
			statusClaudeCode: "已检测到 Claude Code 登录",
			statusAccount: "账号",
			statusPlan: "订阅类型",
			statusTier: "限流档位",
			statusExpiry: "令牌有效期",
			...
			pasteHint:
				"在终端运行 `claude setup-token`（Claude Code 会完成登录），把输出的令牌粘贴到此处保存。这是官方支持的“在 Claude Code 之外使用订阅”的方式。",
		...
		const en = {
			...
			statusPasted: "Using a pasted token (setup-token)",
			statusClaudeCode: "Claude Code login detected",
			...
			pasteHint:
				"Run `claude setup-token` in a terminal (Claude Code handles the login) and paste the token here. This is the supported way to use your subscription outside Claude Code.",
			...
			const statusBody =
				pastedConfigured || status !== undefined
					? h(
							...
							row([
								statusDot(true),
								text(pastedConfigured ? t("statusPasted") : t("statusClaudeCode"), {
									color: "var(--dsw-alias-label-primary)",
								}),
							]),
							status !== undefined
