window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			modeSdk: "Claude Agent SDK（审批模式）",
			modeTokenDesc: "直接调用 Messages API（订阅令牌通常只放行 haiku）",
			modeConsoleDesc: "驱动本机 claude CLI 干活（保留官方客户端资格，opus/sonnet 可用；无审批）",
			modeSdkDesc: "经官方 SDK 托管 Claude Code 代理，工具调用逐一请求你批准（opus/sonnet 可用，有审批）",
			modeConsoleWarn:
			...
			modeSdkWarn:
				"SDK 模式需要本机已安装并登录 claude；每次工具调用（Bash/Edit/WebSearch 等）都会弹出许可请求等你批准或拒绝；工具仍在 Claude Code 沙箱内执行（不是 harness 沙箱），但你有完整的审批与可见性。",
		...
		const en = {
			...
			modeSdk: "Claude Agent SDK (approval)",
			modeTokenDesc: "Call the Messages API directly (subscription tokens usually only allow haiku)",
			modeConsoleDesc: "Drive the local claude CLI (first-party entitlement — opus/sonnet work; no approval)",
			modeSdkDesc: "Host a Claude Code agent via the official SDK; every tool use asks for your approval (opus/sonnet work, with approval)",
			modeConsoleWarn:
			...
			modeSdkWarn:
				"SDK mode needs `claude` installed and logged in. Each tool call (Bash/Edit/WebSearch…) raises a permission request you approve or deny; tools still execute inside Claude Code's sandbox (not the harness sandbox), but you get full approval and visibility.",
		...
		/** Dropdown showing the current value explicitly. */
		const select = (value, onChange, options, ariaLabel, opts) =>
			h(
				"select",
				{
					value,
		...
		function SubscriptionSection(props) {
			...
			const mode = snap.ns?.value?.mode === "console" ? "console" : snap.ns?.value?.mode === "sdk" ? "sdk" : "token";
			...
			return h(
				...
				h(
					...
					text(t("intro")),
					snap.writable === false ? text(t("readOnly"), { color: "var(--dsw-alias-state-warn-primary)", fontSize: "12px" }) : null,
					notice === null ? null : text(notice, { color: "var(--dsw-alias-label-secondary)" }),
					...
					card([
						...
						row([
							select(
								...
								[
									...
									{ value: "console", label: t("modeConsole") },
									{ value: "sdk", label: t("modeSdk") },
								],
								t("modeTitle"),
						...
						mode === "console"
							? text(t("modeConsoleWarn"), { color: "var(--dsw-alias-state-warn-primary)", fontSize: "12px" })
							: mode === "sdk"
								? text(t("modeSdkWarn"), { color: "var(--dsw-alias-state-warn-primary)", fontSize: "12px" })
								: null,
					]),
