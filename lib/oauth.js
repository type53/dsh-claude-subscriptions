...
export class OAuthManager {
  ...
  async beginLogin(flowId) {
    ...
    // Scopes verified against Claude Code's current OAuth flow (issue #22450):
    // `user:inference` is what `claude setup-token` requests to turn the
    // subscription into API access; `user:profile` enables account display.
    url.searchParams.set('scope', 'user:inference user:profile');
