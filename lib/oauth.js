...
export class OAuthManager {
  ...
  async handleCallback(req, res) {
    ...
    try {
      ...
      // pending alive so the final callback can still succeed.
  ...
  /**
   * Resolve the pending login for a state. Strict by default; when the state
   * is unknown and exactly one login is in flight, fall back to it — the
   * magic-link tab may complete with a different state than the popup.
   */
  findPending(state) {
    if (this.pendingByState.has(state)) return this.pendingByState.get(state);
    if (this.pendingByState.size === 1) {
