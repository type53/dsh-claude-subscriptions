...
/** Reasoning effort levels Claude exposes (verified from the claude-code binary). */
...
/**
 * Resolve the modern thinking/effort wire config. Claude's current API uses
 * `thinking: { type: "adaptive" }` plus `output_config: { effort }` (the
 * `effort-2025-11-24` beta; GA on the latest models); `off` omits thinking.
 * @returns `{ thinking, outputConfig }`; both undefined when thinking is off.
 */
function resolveThinking(options, defaults) {
  if (options.purpose === 'session-title') return { thinking: undefined, outputConfig: undefined };
  const enabled = (defaults.thinking ?? 'enabled') === 'enabled';
  const effort = options.reasoningEffort ?? defaults.reasoningEffort;
  if (!enabled || effort === undefined || effort === 'off') return { thinking: undefined, outputConfig: undefined };
  const level = EFFORT_LEVELS.has(effort) ? effort : 'high';
  return {
...
export async function serializeRequest(options, defaults, resolveImage, signal) {
  ...
  const { thinking, outputConfig } = resolveThinking(options, defaults);
  return {
    model: options.model,
    messages,
