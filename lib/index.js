...
export function apply(ctx, config) {
  ...
  // ── settings namespace + OAuth handshake ─────────────────────────────────
  let scope;
  if (settings !== undefined) {
    scope = settings.register(NS, Config, { base: config });
    current = () => scope.get();
    ctx.effect(() => {
      const disposer = scope.watch((next, prev) => {
        runFlowHandshake({ ctx, settings, oauth, scope }, next, prev).catch((error) => {
  ...
  // ── provider route + adapter registration ────────────────────────────────
  let userId;
  const resolveUserId = () => {
  ...
  if (scope !== undefined) {
    ctx.effect(() => scope.watch(() => ensureRegistrationFacts()));
  }
...
async function runFlowHandshake(deps, next, prev) {
  const { ctx, settings, oauth, scope } = deps;
  ...
  try {
    ...
    if (ctx.credentials === undefined) throw new Error('credentials service unavailable');
    const label = await storeTokenResponse(ctx.credentials, tokenResponse);
    await scope.update({
      auth: {
        method: 'oauth',
    ...
    await settings.mutate(NS, [{ op: 'unset', path: ['flow'] }]).catch(() => {});
  }
}
