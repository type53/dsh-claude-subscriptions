...
export function apply(ctx, config) {
  ...
  const adapter = new ClaudeAdapter({
    ...
    resolveUserId,
    logError: (error) => {
      ctx.logger.error('llm-claude: request failed');
      ctx.logger.error(error);
    },
    // SDK mode: route Claude Code tool-permission requests through the
    // harness user-questions channel so the human sees and answers them.
    askUser: async (item) => {
      const userQuestions = ctx.get('userQuestions');
      if (userQuestions === undefined) return undefined;
      try {
        return await userQuestions.ask({ questions: [item] });
      } catch (error) {
        ctx.logger.error('llm-claude: user question failed');
        ctx.logger.error(error);
        return undefined;
      }
    },
    getCwd: () => {
      try {
        ...
        if (workspace?.cwd !== undefined) return workspace.cwd;
      } catch {
