...
async function runFlowHandshake(deps, next, prev) {
  ...
  try {
    ...
    // `update` merges, so it cannot express a removal — passing `auth: undefined`
    // left the previous login's facts on display after a failed one. Same reason
    // the `flow` teardown below uses `mutate`.
    await ctx.settings.mutate(NS, [{ op: 'unset', path: ['auth'] }]).catch(() => {});
    throw error;
  } finally {
