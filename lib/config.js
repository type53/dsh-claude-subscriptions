 ...
 * (`llm-claude.models`) or auto-fetched from the Models API. Catalog
 * membership is advisory — the adapter accepts any model id the endpoint
 * accepts.
 */
 ...
 * absent the section authenticates through the subscription token sources
 * (pasted `claude setup-token` token or the live Claude Code login), and the
 * Models page treats the route as usable without a key.
 */
export const Config = z.object({
  apiKeyEnv: z.string().role('credential-ref'),
  baseURL: z.string().default(DEFAULT_BASE_URL),
  /**
   * `token` = bearer against the Messages API (setup-token / Claude Code
   * login / API key; haiku-only on most subscriptions).
   * `console` = drive the official `claude` CLI (whole-turn agent; opus
   * works; no approval loop).
   * `sdk` = host a Claude Code agent via the Claude Agent SDK with every
   * tool use routed through the harness approval channel (opus works,
   * full approval + trajectory).
   */
  mode: z.union(['token', 'console', 'sdk']).default('token'),
  /**
   * Optional custom system prompt for console mode. When unset, console mode
   * passes NO system prompt: Claude Code uses its own persona, and the
   * harness-rendered prompt (which carries harness branding/instructions) is
   * not injected into the claude process.
   */
  consoleSystemPrompt: z.string(),
  thinking: z.union(['enabled', 'disabled']).default('enabled'),
  // Reasoning effort levels Claude exposes (verified from the claude-code
  // binary: low / medium / high / xhigh / max, plus off to disable thinking).
  reasoningEffort: z.union(['off', 'low', 'medium', 'high', 'xhigh', 'max']).default('high'),
  ...
  /** Host-mirrored subscription status for the Subscriptions tab. */
  status: z.object({
    source: z.string(),
    account: z.string(),
  ...
  /** Browser-written refresh marker: the host recomputes `status` and clears it. */
  refresh: z.object({
    at: z.number(),
  }),
});
...
export function resolveAdapterOptions(config) {
  ...
  return {
    ...
    mode: config.mode === 'console' ? 'console' : config.mode === 'sdk' ? 'sdk' : 'token',
    consoleSystemPrompt:
      config.consoleSystemPrompt !== undefined && config.consoleSystemPrompt.length > 0
        ? config.consoleSystemPrompt
        : undefined,
    defaults: {
